...
class Navigator(HighchartsMeta):
    ...
    def outline_color(self, value):
        if not value:
            ...
            try:
                self._outline_color = Gradient.from_json(value)
            except ValueError:
                if isinstance(value, dict):
                    self._outline_color = Gradient.from_dict(value)
                else:
                    self._outline_color = validators.string(value)
        elif isinstance(value, dict) and 'linear_gradient' in value:
            self._outline_color = Gradient(**value)
        elif isinstance(value, (dict, str)) and 'patternOptions' in value:
            try:
                self._outline_color = Pattern.from_json(value)
            except ValueError:
                if isinstance(value, dict):
                    self._outline_color = Pattern.from_dict(value)
                else:
                    self._outline_color = validators.string(value)
        elif isinstance(value, dict) and 'pattern_options' in value:
            self._outline_color = Pattern(**value)
        else:
            raise errors.HighchartsValueError(f'Unable to resolve value to a string, '
                                              f'Gradient, or Pattern. Value received '
                                              f'was: {value}')
