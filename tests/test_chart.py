...
@pytest.mark.parametrize(
    "input_filename, expected_filename, as_file, error",
    [
        ("chart_obj/01-input.js", "chart_obj/01-expected.js", False, None),
        ("chart_obj/01-input.js", "chart_obj/01-expected.js", False, None),
    ],
)
def test_from_js_literal(
    input_files, input_filename, expected_filename, as_file, error
):
    Class_from_js_literal_with_expected(
        cls, input_files, input_filename, expected_filename, as_file, error
    )


@pytest.mark.parametrize(
    "json_str, expected_modules, error",
    [
        (
            """{
    "chart": {
    ...
    "yAxis": [{
        ...
        "title": {
            ...
            ["highcharts", "modules/exporting", "modules/pareto"],
            None,
        ),
    ],
)
...
@pytest.mark.parametrize(
    "options_str, as_str, expected, error",
    [
        (
            """{
        "chart": {
...
@pytest.mark.parametrize(
    "kwargs, expected_series, expected_data_points, error",
    [
        ({}, 0, [], None),
        ({"series": [{"data": [[1, 2], [3, 4]], "type": "line"}]}, 1, [(0, 2)], None),
        ({"series": {"data": [[1, 2], [3, 4]], "type": "line"}}, 1, [(0, 2)], None),
        ({"data": [[1, 2], [3, 4]], "series_type": "line"}, 1, [(0, 2)], None),
        (
            {
                "data": [[1, 2], [3, 4]],
            },
...
def test_issue90_one_shot_creation(
    kwargs, expected_series, expected_data_points, error
):
    if not error:
        result = cls(**kwargs)
...
@pytest.mark.parametrize(
    "filename, kwargs, pre_test_df_func, expected_series, expected_data_points, error",
    [
        ...
        (
            "test-data-files/nst-est2019-01.csv",
            {
                "property_map": {"x": ["Geographic Area", "2010"]},
                "series_in_rows": False,
            },
            None,
            2,
        ...
        ("test-data-files/nst-est2019-01.csv", {}, reduce_to_two_columns, 1, 57, None),
        # SCENARIO 3b: Multiple Series, Multipled Columns
        ("test-data-files/nst-est2019-01.csv", {}, prep_df, 10, 57, None),
        ...
        ("test-data-files/nst-est2019-01.csv", {}, None, 11, 57, TypeError),
    ],
)
...
@pytest.mark.parametrize(
    "filename, expected_series, expected_data_points, error",
    [
        ("test-data-files/nst-est2019-01.csv", 57, 10, None),
    ],
)
def test_from_csv_in_rows(
    input_files, filename, expected_series, expected_data_points, error
):
    input_file = check_input_file(input_files, filename)
    if not error:
        result = cls.from_csv_in_rows(input_file, wrapper_character='"')
        assert result is not None
        assert isinstance(result, cls)
...
@pytest.mark.parametrize(
    "filename, property_map, kwargs, expected_series, expected_data_points, error",
    [
        (
            "test-data-files/nst-est2019-01.csv",
            {},
        ...
        (
            "test-data-files/nst-est2019-01.csv",
            {"name": "Geographic Area", "x": "Geographic Area", "y": "2010"},
            {"wrapper_character": '"'},
            1,
            57,
            None,
        ...
        (
            "test-data-files/nst-est2019-01.csv",
            {},
            {"wrapper_character": '"', "series_in_rows": True},
            57,
            10,
            None,
        ),
        ...
        (
            "test-data-files/nst-est2019-01.csv",
            {"name": "Geographic Area"},
            {"wrapper_character": '"', "series_in_rows": False},
            1,
            57,
            None,
        ),
        ...
        (
            ...
            {"series_in_rows": False, "wrapper_character": '"'},
            2,
            57,
            None,
        ),
        ...
        (
            "test-data-files/nst-est2019-01.csv",
            {},
            {"wrapper_character": '"', "x": "Geographic Area", "y": "2010"},
            1,
            57,
            None,
        ),
...
def test_from_csv(
    ...
    if not error:
        result = cls.from_csv(input_file, property_column_map=property_map, **kwargs)
        assert result is not None
        assert isinstance(result, cls)
        assert result.options is not None
        assert result.options.series is not None
        ...
        with pytest.raises(error):
            result = cls.from_csv(
                input_file, property_column_map=property_map, **kwargs
            )
...
def test_from_array(value, expected_shape, has_ndarray, has_data_points, error):
    if has_ndarray is False and has_data_points is False:
        raise AssertionError(
            "Test is invalid. has_ndarray or has_data_points must be "
...
@pytest.mark.parametrize(
    ...
    [
        (None, None),
        ("https://mycustomurl.com/", None),
        ("../some/relative/path", None),
    ],
)
def test_chart_module_url(module_url, error):
    if not error:
        result = cls(module_url=module_url)
        assert result is not None
        if module_url is not None:
            assert result.module_url == module_url
            if module_url != "https://code.highcharts.com/":
                assert result.module_url != "https://code.highcharts.com/"
        else:
            assert result.module_url == "https://code.highcharts.com/"
    else:
        with pytest.raises(error):
